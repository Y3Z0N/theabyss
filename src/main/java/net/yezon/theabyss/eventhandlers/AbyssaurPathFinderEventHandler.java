...
public class AbyssaurPathFinderEventHandler {
		...
		if (Math.random() < 0.003) {
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("theabyss:abyssaur_attack")), SoundSource.NEUTRAL, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("theabyss:abyssaur_attack")), SoundSource.NEUTRAL, 1, 1, false);
				}
			}
		}
	}
