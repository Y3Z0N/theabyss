...
import net.yezon.theabyss.entity.renderer.NightbladeBossRenderer;
...
public class NightbladeBossEntity extends TheAbyss.Processor {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).immuneToFire()
			.size(0.8f, 1.7999999999999998f)).build("nightblade_boss").setRegistryName("nightblade_boss");

	public NightbladeBossEntity(TheAbyss instance) {
		super(instance, 384);
		FMLJavaModLoadingContext.get().getModEventBus().register(new NightbladeBossRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -16777216, -16777216, new Item.Properties().group(TheAbyssEntityItemGroup.tab))
				.setRegistryName("nightblade_boss_spawn_egg"));
	}
	...
	public static class CustomEntity extends MonsterEntity {
		...
		public CustomEntity(EntityType<CustomEntity> type, World world) {
			...
			setNoAI(false);
			setCustomName(new StringTextComponent("Nightblade"));
			setCustomNameVisible(true);
			enablePersistence();
			this.setItemStackToSlot(EquipmentSlotType.MAINHAND, new ItemStack(AbyssSwordItem.block));
			this.setItemStackToSlot(EquipmentSlotType.OFFHAND, new ItemStack(AbyssSwordItem.block));
		}
		...
		protected void dropSpecialItems(DamageSource source, int looting, boolean recentlyHitIn) {
			...
			this.entityDropItem(new ItemStack(AbyssSwordItem.block));
		}
		...
		public boolean attackEntityFrom(DamageSource source, float amount) {
			...
			NightbladeBossDodgeOnHurtEvent.executeEvent(Stream
		...
		public void onDeath(DamageSource source) {
			...
			NightbladeBossDiesEvent.executeEvent(Stream
					.of(new AbstractMap.SimpleEntry<>("world", world), new AbstractMap.SimpleEntry<>("x", x), new AbstractMap.SimpleEntry<>("y", y),
		...
		public ILivingEntityData onInitialSpawn(IServerWorld world, DifficultyInstance difficulty, SpawnReason reason,
			...
			NightBladeMusicOnSpawnEvent.executeEvent(Stream
					.of(new AbstractMap.SimpleEntry<>("world", world), new AbstractMap.SimpleEntry<>("x", x), new AbstractMap.SimpleEntry<>("y", y),
							new AbstractMap.SimpleEntry<>("z", z), new AbstractMap.SimpleEntry<>("entity", entity))
					.collect(HashMap::new, (_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll));
			return retval;
		...
		public void baseTick() {
			...
			NightBladeBossAbilityEvent.executeEvent(Stream
					.of(new AbstractMap.SimpleEntry<>("world", world), new AbstractMap.SimpleEntry<>("x", x), new AbstractMap.SimpleEntry<>("y", y),
							new AbstractMap.SimpleEntry<>("z", z), new AbstractMap.SimpleEntry<>("entity", entity))
					.collect(HashMap::new, (_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll));
		}
