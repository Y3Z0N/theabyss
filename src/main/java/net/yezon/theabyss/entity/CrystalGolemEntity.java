...
public class CrystalGolemEntity extends Monster {
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("theabyss:slime_forest"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(TheabyssModEntities.CRYSTAL_GOLEM.get(), 1, 1, 1));
	}
	...
	public CrystalGolemEntity(EntityType<CrystalGolemEntity> type, Level world) {
		super(type, world);
		xpReward = 50;
		setNoAi(false);
		setCustomName(new TextComponent("Crystal Golem"));
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(TheabyssModItems.CRYSTAL_HAND.get()));
	}

	@Override
	public SoundEvent getAmbientSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("theabyss:crystal_golem_idle"));
	}
	...
	public SoundEvent getHurtSound(DamageSource ds) {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("theabyss:crystal_golem_hurt"));
	}

	@Override
	public SoundEvent getDeathSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("theabyss:crystal_golem_die"));
	}
