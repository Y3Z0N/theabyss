...
public class TransformationOverlay {
	...
	public static void eventHandler(RenderGameOverlayEvent.Post event) {
		if (event.getType() == RenderGameOverlayEvent.ElementType.HELMET) {
			...
			double y = _y;
			double z = _z;
			float Infected = ((float) ((entity.getCapability(Capabilities.PLAYER_VARIABLES_CAPABILITY, null)
			.orElse(new Capabilities.PlayerVariables())).Infected));
			...
			RenderSystem.color4f(Infected, Infected, Infected, Infected);
			RenderSystem.disableAlphaTest();
			if (true) {
				Minecraft.getInstance().getTextureManager().bindTexture(new ResourceLocation("theabyss:textures/transformation.png"));
