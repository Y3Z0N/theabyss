...
public class ResearchTableBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(1, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ResearchTableBlockEntity(BlockPos position, BlockState state) {
		super(TheabyssModBlockEntities.RESEARCH_TABLE.get(), position, state);
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new ResearchMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
