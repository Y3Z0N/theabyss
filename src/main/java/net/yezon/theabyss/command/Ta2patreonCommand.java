...
import net.yezon.theabyss.eventhandlers.ExecutePatreonCommandEventHandler;
...
public class Ta2patreonCommand {
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("ta2patreon")
				...
				.then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(arguments -> {
					...
					ExecutePatreonCommandEventHandler.execute(world, x, y, z, entity);
					return 0;
				})).executes(arguments -> {
					ServerLevel world = arguments.getSource().getLevel();
					double x = arguments.getSource().getPosition().x();
