...
public class TheAbyssJungleBiome extends TheAbyss.Processor {
	public static Biome biome;

	public TheAbyssJungleBiome(TheAbyss instance) {
		super(instance, 562);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-15129037).setWaterColor(-13421773).setWaterFogColor(-13421773)
						.withSkyColor(-15129037).withFoliageColor(-14473931).withGrassColor(-13158071)
						.setAmbientSound((net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation("theabyss:blue_jungle_ambience")))
						...
						.setParticle(new ParticleEffectAmbience(WandThunderPTParticle.particle, 0.018f)).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.4600000000000001f).scale(0.3f)
						.temperature(0.7f).downfall(0.3f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("theabyss:blue_jungle"));
			}
		}
	}
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("blue_jungle_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("blue_jungle_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("blue_jungle_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
					...
					for (Direction direction : Direction.Plane.HORIZONTAL) {
						if (p_225576_2_.nextFloat() <= 0.25F) {
							...
							if (Feature.isAirAt(p_225576_1_, blockpos)) {
								BlockState blockstate = JungleLeavesBlock.block.getDefaultState();
