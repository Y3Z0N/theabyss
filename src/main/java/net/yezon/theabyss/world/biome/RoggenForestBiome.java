...
import net.yezon.theabyss.particle.WandThunderPTParticle;
...
public class RoggenForestBiome extends TheAbyss.Processor {
	public static Biome biome;

	public RoggenForestBiome(TheAbyss instance) {
		super(instance, 814);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-3345409).setWaterColor(-11817068).setWaterFogColor(-13213597)
						.withSkyColor(-3345409).withFoliageColor(-6180170).withGrassColor(-15774392)
						.setAmbientSound((net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation("theabyss:biome_ruma_ambience")))
						.setMusic(new BackgroundMusicSelector((net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation("theabyss:biome_ruma_music")), 12000, 24000, true))
						.setParticle(new ParticleEffectAmbience(WandThunderPTParticle.particle, 0.03f)).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(TantraGrassBlock.block.getDefaultState(),
								TantraDirtBlock.block.getDefaultState(), TantraDirtBlock.block.getDefaultState())));
								...
								Feature.DISK
										.withConfiguration(
												new SphereReplaceConfig(Blocks.GRAVEL.getDefaultState(), FeatureSpread.func_242253_a(2, 3), 2,
														ImmutableList.of(TantraGrassBlock.block.getDefaultState(),
																TantraDirtBlock.block.getDefaultState())))
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.25f).scale(0.2f)
						.temperature(0.5f).downfall(0.3f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("theabyss:roggen_forest"));
			}
		}
	}
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("roggen_forest_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("roggen_forest_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("roggen_forest_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
					...
					for (Direction direction : Direction.Plane.HORIZONTAL) {
						if (p_225576_2_.nextFloat() <= 0.25F) {
							...
							if (Feature.isAirAt(p_225576_1_, blockpos)) {
								BlockState blockstate = RoggenLeaveBlock.block.getDefaultState();
