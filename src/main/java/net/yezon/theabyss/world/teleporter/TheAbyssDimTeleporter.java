...
public class TheAbyssDimTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_abyss_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("the_abyss_portal",
				com.google.common.collect.Sets
						.newHashSet(ImmutableSet.copyOf(TheabyssModBlocks.THE_ABYSS_PORTAL.get().getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("the_abyss_portal");
	...
	public TheAbyssDimTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = TheAbyssDimPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return TheAbyssDimPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
