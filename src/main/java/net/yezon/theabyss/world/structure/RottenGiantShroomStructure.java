...
public class RottenGiantShroomStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 50000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == GrassSmallBlock.block)
								blockCriteria = true;
							if (blockAt.getBlock() == GrassMiddleBlock.block)
								blockCriteria = true;
							if (blockAt.getBlock() == GrassBigBlock.block)
								blockCriteria = true;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("theabyss", "giant_shroom_rotten"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("rotten_giant_shroom"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("theabyss:rotten_giant_shroom"), configuredFeature);
		}
	}
