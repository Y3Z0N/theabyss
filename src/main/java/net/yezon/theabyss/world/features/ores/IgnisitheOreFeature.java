...
public class IgnisitheOreFeature extends OreFeature {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("theabyss:ignisithe_ore", FEATURE,
				new OreConfiguration(List.of(OreConfiguration.target(new BlockStateMatchTest(TheabyssModBlocks.STONE.get().defaultBlockState()), TheabyssModBlocks.IGNISITHE_ORE.get().defaultBlockState())), 5));
		PLACED_FEATURE = PlacementUtils.register("theabyss:ignisithe_ore", CONFIGURED_FEATURE,
				List.of(CountPlacement.of(2), InSquarePlacement.spread(), HeightRangePlacement.uniform(VerticalAnchor.absolute(-40), VerticalAnchor.absolute(-20)), BiomeFilter.biome()));
		return FEATURE;
	}
