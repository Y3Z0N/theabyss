...
public class JungleTreeFeature extends Feature<NoneFeatureConfiguration> {
	public static JungleTreeFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new JungleTreeFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("theabyss:jungle_tree", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("theabyss:jungle_tree", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("theabyss", "jungle_tree"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 750000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (!base_blocks.contains(context.level().getBlockState(new BlockPos(i, j, k)).getBlock()))
					continue;
				BlockPos spawnTo = new BlockPos(i + -6, j + -5, k + -6);
