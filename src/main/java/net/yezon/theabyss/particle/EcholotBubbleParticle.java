...
public class EcholotBubbleParticle {
	...
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("echolot_bubble"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.particleScale *= (float) 1.7;
			this.maxAge = (int) Math.max(1, 146 + (this.rand.nextInt(12) - 6));
			this.particleGravity = (float) -0.04;
			this.canCollide = true;
			this.motionX = vx * 0.008;
			this.motionY = vy * 0.008;
			this.motionZ = vz * 0.008;
